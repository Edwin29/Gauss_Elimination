...
class GaussElimination:
    def __init__(self, rows, cols, step, max_no=20):
        self.rows = rows
        self.cols = cols
        self.max_no = max_no
    ...
    #랜덤 행렬 생성(-20~20 사이의 정수)
    def random_matrix(rows,cols, max_no=20):
        row_list=[]
        for r in range(rows):
            col_list=[]
            for c in range(cols):
                col_list.append(float(random.randint(-max_no,max_no))) #float형으로 변환함
    ...
    def gauss_jordan_elimination(matrix):
        ...
        for col_count in range(mcol-1): #선행 원소 아래의 값(열 하단)이 0이 될때까지 반복 수행
            ...
            step = step + 1
            print("{})".format(step), first_row+1,"행이", row_count+1,"행과 교환")
        ...
        for col in range(mcol-2,0,-1):
            for row in range(col-1,-1,-1): #col/row를 파이썬 인덱스에 맞게 적용
                ...
                step = step + 1
                print("{})".format(step), col+1,"행을",const, "만큼 상수배하여", row+1, "행의", col+1,"열을 소거") #
                matrix[row] = matrix[row] + const * matrix[col]
                
                print(matrix, "\n")
        
        round = 1
        X = np.round(matrix[:,-1],round)
        print("방정식의 해는")
        print(X,"\n(소수점 {}자리까지 반올림)".format(round))
